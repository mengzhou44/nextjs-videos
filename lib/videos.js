...
export const getVideos = async (searchQuery) => {
  ...
  const url = `${BASE_URL}/search?part=snippet&q=${searchQuery}&type=video&key=${YOUTUBE_API_KEY}`;
...
export const getPopularVideos = async () => {
  const YOUTUBE_API_KEY = process.env.YOUTUBE_API_KEY;
  const url = `${BASE_URL}/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&regionCode=US&key=${YOUTUBE_API_KEY}`;
  return await fetchVideos(url);
};
